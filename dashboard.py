...
HOF_TIER = {
    ...
    "hexa_rainbow_spectrum_bodypaint",
    "trio_inside_outside_bodypaint",        # 6인 무지개 세로4:5 HOF급
}
...
with st.sidebar:
    st.markdown("### ?? 전역 설정")
    st.markdown("---")
    global_platform = st.radio("??? 출력 플랫폼", options=["Gemini", "ChatGPT (DALL-E)", "Midjourney"], index=0)
...
tab1, tab2, tab3, tab4 = st.tabs(["?? 프리셋 모드", "??? 수동 조합", "?? 랜덤 모드", "?? 영상 프롬프트"])
...
""", unsafe_allow_html=True)
    ...
    with col2:
        ...
        preset_framing     = st.selectbox("??? 프레이밍",        [NONE] + list(FRAMING.keys()),          key="preset_framing")
    ...
    with col3:
        preset_environment = st.selectbox("??? 환경",            [NONE] + list(ENVIRONMENTS.keys()),     key="preset_environment")
        preset_weather     = st.selectbox("??? 날씨",            [NONE] + list(WEATHER.keys()),          key="preset_weather")
...
with tab2:
    ...
    with col1:
        ...
        body_weight = st.selectbox("?? 체형 보정",                 list(BODY_WEIGHT.keys()),       index=idx(BODY_WEIGHT,      "r_body_weight"))
    ...
    with col2:
        st.markdown("##### ?? 스타일")
        use_separate = st.checkbox("?? 상하의 분리 선택", value=False, key="use_separate_outfit", help="상의+하의를 각각 선택해 조합")
        if use_separate:
...
""", unsafe_allow_html=True)
    ...
    with col3:
        st.markdown("##### ??? 환경")
        environment = st.selectbox("??? 촬영 장소",                list(ENVIRONMENTS.keys()),      index=idx(ENVIRONMENTS,     "r_env"))
        weather     = st.selectbox("??? 날씨/기상",                 list(WEATHER.keys()),           index=idx(WEATHER,          "r_weather"))
        ...
        framing     = st.selectbox("??? 프레이밍 ? 구도/크기",      list(FRAMING.keys()),           index=idx(FRAMING,          "r_framing"))
        ...
        color_grade = st.selectbox("??? 색감 ? 컬러 그레이딩",     list(COLOR_GRADES.keys()),      index=idx(COLOR_GRADES,     "r_color_grade"))
...
{{"risk_level": "HIGH/MEDIUM/LOW","issues": ["issue1"],"replacements": {{"outfit": "key or null","material": "key or null","angle": "key or null","pose": "key or null","skin_tone": "key or null","body_oil": "key or null","weather": "key or null","style": "key or null","img_style": "key or null"}},"summary": "한국어 2-3줄"}}"""}]
    ...
    if st.session_state.get("_trigger_build", False):
        ...
        auto_labels = {"r_pose":"?? 포즈","r_expression":"?? 표정","r_skin_tone":"?? 피부","r_hair_style":"?? 헤어","r_hair_color":"?? 헤어컬러","r_makeup":"?? 메이크업","r_footwear":"?? 신발","r_color_grade":"??? 색감","r_accessories":"?? 액세서리","r_body_oil":"? 바디오일","r_weather":"??? 날씨","r_bg_crowd":"?? 배경","r_tattoo":"?? 문신","r_special_effects":"?? 특수효과","r_props":"?? 소품","r_image_style":"?? 이미지스타일","r_era":"?? 시대","r_concept":"?? 컨셉"}
        picked_items = {}
...
with tab4:
    ...
    with col1:
        ...
        video_duration = st.selectbox("?? 영상 길이", list(VIDEO_DURATIONS.keys()))
